...
import { EMessageSender, EMessageStatus } from "../../utils/vars";
import { TMessageIncoming } from "../../utils/types";

type TProps = {
    message: TMessageIncoming;
};

const Message: FC<TProps> = ({ message }) => {
    const time = useMemo(
        () => new Date(message.timestamp * 1000).toLocaleString() + "",
        [message.timestamp]
    );

    const messageOutgoing = message.type === EMessageSender.OUTGOING;
    const messageSent = message.statusMessage === EMessageStatus.SENT;
    const messageDelivered = message.statusMessage === EMessageStatus.DELIVERED;
    const messageRead = message.statusMessage === EMessageStatus.READ;

    const messageSender = messageOutgoing
        ? "you"
        : message.senderName || message.senderId;

    const svgFill = messageDelivered
    ...
    return (
        <article
            className={`${styles.item} ${
                messageOutgoing ? styles.right : styles.left
            }`}
        >
            <span className={styles.type}>{messageSender}</span>
